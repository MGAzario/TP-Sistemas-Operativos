...
=======
int main(int argc, char *argv[])
>>>>>>> 4fbe0f7c213bc80cd2d83c2883a61b4ce0d41807
{
...
void crear_interfaz_generica(char *nombre)
{
    ...
    while (true)
    {
        ...
        if (recibir_operacion(socket_kernel) == IO_GEN_SLEEP)
        {
            t_list peticion_kernel = list_create();
            peticion_kernel = recibir_paquete(socket_kernel);
            char *nombre_interfaz = list_take(peticion_kernel, 1);
            u_int32_t unidades_de_trabajo = list_take(peticion_kernel, 2);
            int multiplicador = atoi(list_take(peticion_kernel, 2));
            usleep(multiplicador * 1000 * tiempo_unidad_trabajo);
            list_destroy(peticion_kernel);
        }
        else {
